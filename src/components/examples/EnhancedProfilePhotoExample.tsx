...
export function EnhancedProfilePhotoExample() {
  const { user } = useAuth();
  ...
  const handlePickImage = async () => {
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        // Upload using enhanced profile photo upload
        ...
        if (uploadResult.success) {
          ...
          const currentPhotos = await EnhancedPhotoUploadService.getUserProfilePhotosFromProfileBucket(user.id);
          const updatedPhotos = [...currentPhotos, uploadResult.url!];
          await EnhancedPhotoUploadService.updateProfilePhotosArrayInDatabase(user.id, updatedPhotos);
    ...
    try {
      ...
      if (!result.canceled && result.assets && result.assets.length > 0) {
        ...
        if (uploadResult.success) {
          ...
          Alert.alert('Success', 'Profile photo taken and uploaded successfully!');
        } else {
          Alert.alert('Upload Failed', uploadResult.error || 'Unknown error');
        }
      }
        ...
        { 
          ...
          onPress: async () => {
            try {
              ...
              const success = await EnhancedPhotoUploadService.deleteProfilePhotoFromProfileBucket(storagePath);
  ...
  const loadUserPhotos = async () => {
    ...
    try {
      const photos = await EnhancedPhotoUploadService.getUserProfilePhotosFromProfileBucket(user.id);
  ...
  return (
    <ScrollView style={styles.container}>
      <Text style={styles.title}>Enhanced Profile Photo Upload</Text>
